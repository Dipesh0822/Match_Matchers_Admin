...
import {
  IconLock as ChangePasswordIcon,
  IconX as CancelIcon,
} from "@tabler/icons";
...
function ChangePassword() {
  const { userdata } = JSON.parse(localStorage.getItem("MATCH_MASTER"));
  ...
  const changePassword = async ({ email, password }) => {
    ...
    const response = await fetch(
      `${API_URL}${InternalAPI.PASSWORDCHANGE}`,
      requestOptions
    );
  ...
  return (
    <Box>
                  ...
                  <Grid item>
                    ...
                    >
                      ...
                      {formik.touched.password && formik.errors.password && (
                        <FormHelperText id="question-error">
      ...
      {res.status === "failed" && (
        <AlertComponent status="false" message={res.msg} />
      )}
      {res.status === "success" && (
        <AlertComponent status="true" message={res.msg} />
      )}
    </Box>
  );
}
